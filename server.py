...
def add_color(tweets):
    ...
    for t in tweets:
        score = t['score']
        color_index = round((score + 1) * 49.5)
        t['color'] = str((colors[int(color_index)]))
...
def tweets(name):
    ...
    scores = [tweet['score'] for tweet in record['tweets']]
    median_score = median(scores)

    return render_template("tweets.html", record=record, median_score = median_score)
...
def following(name):
    ...
    """
    followed_by = fetch_following(api, name)

    return render_template("following.html", name=name, followed_by=followed_by)
